...
describe('NativeChatAugmenter ghost blocks', () => {
  ...
  it('anchors removed banned rows, reasserts after recycle, reanchors, falls back, and keeps deletes intact', async () => {
    ...
    expect(document.querySelector('.kickflow-ghost-strip')?.textContent).toContain('first banned');
    expect(augmenter.getGhostStats()).toMatchObject({ ghostAnchored: 0, ghostPendingNoAnchor: 2, ghostStrip: 2 });
