...
export interface RenderQueueOptions {
  ...
  // wasAtBottom is measured BEFORE the batch is appended — a large 250ms/50-message batch
  // can grow scrollHeight past the near-bottom threshold, so measuring after append would
  // wrongly conclude the user had scrolled up and break live bottom-follow.
  onFlush?: (appended: HTMLElement[], wasAtBottom: boolean) => void;
}

/** Batches incoming messages: flush on a 250ms timer OR once >=50 are pending, whichever
 * comes first, then appends a single DocumentFragment inside a requestAnimationFrame
 * callback. Keeps chat rendering off the hot path of every individual Pusher message. */
